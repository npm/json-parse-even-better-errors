...
const jsonThrows = (data, ...args) => {
  ...
  // Handle custom validation for systemError property
  const systemErrorConstructor = expected?.systemError
  const validate = (err) => {
    if (expected.message) {
    ...
    if (expected.position !== undefined && err.position !== expected.position) {
      return false
    }
    if (systemErrorConstructor && !(err.systemError instanceof systemErrorConstructor)) {
      return false
    }
    return true
  }

  assert.throws(() => parseJson(data, null, context), validate)
}
